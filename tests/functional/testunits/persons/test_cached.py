...
async def test_person_search_uses_cache(
    make_get_request, target_person_data, es_destroy_mock_data, clear_cache_by_prefix
):
...
async def test_persons_films_uses_cache(
    ...
    films_from_es, _ = await make_get_request(f"/api/v1/persons/{target_person_data['id']}/films")
    ...
    await clear_cache_by_prefix(es_test_settings.persons_index)
    films_deleted, _ = await make_get_request(f"/api/v1/persons/{target_person_data['id']}/films")
...
async def test_person_details(
    ...
    person_from_es, _ = await make_get_request(f"/api/v1/persons/{target_person_data['id']}")

    assert person_from_es["uuid"] == target_person_data["id"]

    await es_destroy_mock_data(es_test_settings.persons_index)
    person_from_cache, _ = await make_get_request(f"/api/v1/persons/{target_person_data['id']}")

    assert person_from_cache["uuid"] == target_person_data["id"]

    await clear_cache_by_prefix(es_test_settings.persons_index)
    _, status_not_found = await make_get_request(f"/api/v1/persons/{target_person_data['id']}")
