...
async def test_genres_get_by_id(es_mock_data, make_get_request):
    ...
    body, genre_found_status = await make_get_request(f"/api/v1/genres/{ES_DATA[0]['id']}")
    body, genre_not_found_status = await make_get_request(f"/api/v1/genres/{NON_EXISTENT_ID}")
