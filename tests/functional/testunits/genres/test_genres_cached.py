...
async def test_genre_get_by_id_cache(
    make_get_request, es_mock_data, es_destroy_mock_data, clear_cache_by_prefix
):
    ...
    empty_body, status = await make_get_request("/api/v1/genres")
...
async def test_genres_all_cache(
    make_get_request, es_mock_data, es_destroy_mock_data, clear_cache_callable
):
    await es_mock_data(es_test_settings.genres_index, ES_DATA)
