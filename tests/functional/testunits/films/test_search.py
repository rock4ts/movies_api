...
async def test_search(es_mock_data, make_get_request, query_data, expected_answer):
...
@pytest.mark.asyncio(loop_scope="session")
async def test_search_pages(es_mock_data, make_get_request, query_data, expected_answer):
...
async def test_search_cached(
    es_mock_data, es_destroy_mock_data, clear_cache_by_prefix, make_get_request
):
    ...
    es_body, status = await make_get_request("/api/v1/films/search", query_data)
    ...
    cached_body, status = await make_get_request("/api/v1/films/search", query_data)
    ...
    empty_body, status = await make_get_request("/api/v1/films/search", query_data)
