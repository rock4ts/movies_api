...
import app.api.v1.response_models as response_models
...
class FilmDetailResponse:
    body = {
        ...
        "writers": [{"id": uuid.uuid4(), "name": "William Smith"}],
        "directors": [{"id": uuid.uuid4(), "name": "Michael Bay"}],
    }
    response = response_models.FilmDetail.model_validate(body)
...
class FilmListResponse:
    ...
    response = [response_models.Film.model_validate(item) for item in body]
...
class GenreListResponse:
    ...
    response = [response_models.Genre.model_validate(item) for item in body]
...
class PersonSearchResponse:
    ...
    response = [response_models.Person.model_validate(item) for item in body]
...
class PersonDetailResponse:
    ...
    response = response_models.Person.model_validate(body)
...
class GenreDetailResponse:
    body = {"id": uuid.uuid4(), "name": "Action"}
    response = response_models.Genre.model_validate(body)
